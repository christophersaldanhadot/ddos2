...
bot = telebot.TeleBot('7143587146:AAH_k1Im0bw4b4Ci4qr8SuqvtWr79z6CFWU')
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"{username}, 𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃.🔥🔥\n\n𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n𝐏𝐨𝐫𝐭: {port}\n𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n𝐌𝐞𝐭𝐡𝐨𝐝: VIP- User of @christophersaldanha"
    bot.reply_to(message, response)
...
def handle_bgmi(message):
    ...
    if user_id not in allowed_user_ids:
        response = (
            ...
            "DM TO BUY ACCESS:- @christophersaldanha"
        )
        bot.reply_to(message, response)
        return
    ...
    if user_id not in admin_id:
        if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < COOLDOWN_TIME:
            response = "❌ You Are On Cooldown. Please wait 10 seconds before using /bgmi again."
            bot.reply_to(message, response)
            return
...
Buy From :- @christophersaldanha
Official Channel :- @christophersaldanha
'''
...
3. MAKE SURE YOU JOINED text @christophersaldanha OTHERWISE NOT WORK
